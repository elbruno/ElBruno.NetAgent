...
namespace ElBruno.NetAgent.Tests
{
    public class SettingsViewModelAsyncTests
    {
        ...
        {n            public NetAgentOptions OptionsToReturn { get; set; }
...
n            // Measure Execute(...) return time - it should not block while save runs
            var sw = Stopwatch.StartNew();
            vm.SaveCommand.Execute(null);
            sw.Stop();
n            Assert.True(sw.ElapsedMilliseconds < 200, $"SaveCommand.Execute blocked the caller for {sw.ElapsedMilliseconds}ms");
n            // Verify the save actually completed within a reasonable timeout (2s)
        ...
        private class FakeInventory : INetworkInventoryService
        {
            ...
            public Task<NetworkInterfaceInfo[]> GetInterfacesAsync(CancellationToken cancellationToken = default)
            {
                Called = true;
                return Task.FromResult(Array.Empty<NetworkInterfaceInfo>());
            }
        }
n        private class DummyQualityMonitor : INetworkQualityMonitor
        {            public Task<NetworkQualityReport> EvaluateAsync(NetworkInterfaceInfo adapter, CancellationToken cancellationToken = default)
            {                return Task.FromResult(new NetworkQualityReport { InterfaceId = adapter.Id, LatencyMs = 0, PacketLoss = 0.0 });            }
        }
        private class DummyDecisionEngine : IDecisionEngine
        {            public Task<Core.Decision.DecisionResult> EvaluateAsync(NetworkQualityReport report, NetAgentOptions options, CancellationToken cancellationToken = default)            {                return Task.FromResult(new Core.Decision.DecisionResult { Action = Core.Decision.DecisionAction.NoAction, Reason = "test" });            }
        }
